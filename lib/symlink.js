...
var analyze = function (deps, absPaths, globals, names, options) {
  ...
  // Oz: Calculate the dependents of each package
  // This is the inverse of deps (dependencies).
  const dependents = {}
  Object.keys(deps).forEach(function (dependent) {
    deps[dependent].forEach(function (dependency) {
      var arr = dependents[dependency]

      if (!arr) dependents[dependency] = [dependent]
      else if (arr.indexOf(dependency) < 0) arr.push(dependent)
    })
  })
  ...
  sorted.forEach(function (n) {
    ...
    // Oz
    var hasDependents = !!dependents[n]
    if (hasDependents && !options.includeLibs) return
    if (!hasDependents && !options.includeApps) return
    ...
    if (linked.length > 0) {
      ...
      // By Oz, not using // cmds.push(cd + 'npm install --no-progress && ' + 'npm link ' + linked.join(' '));
    }
    ...
    if (dependents[n]) { // Oz: only do this if the library has dependents since npm link can take a long time.
...
module.exports = function (dirs, globals, options, cb) {
  ...
  getJsonsFromDirectories(dirs, function (err, datas) {
    ...
    var cmds;
    try {
      cmds = analyze(deps, absPaths, globals, names, options);
    }
    catch (err) {
      return cb(err);
    }
